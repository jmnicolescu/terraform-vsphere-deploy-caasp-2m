...
module "caasp4-admin" {
    ...
    guest_vm_name           = "caasp4-admin"
    guest_vcpu              = "4"
    guest_memory            = "16384"
    guest_disk0_size        = "40"
    guest_network           = "lab-mgmt"
    guest_ipv4_address      = "192.168.120.110"
...
output "caasp4-admin-VM-ip" {
	value = module.caasp4-admin.VM-ip
}
output "caasp4-admin-VM-guest-ip" {
	value = module.caasp4-admin.VM-guest-ip
}
