...
# -------------------------------------------------------------
# Create server - caasp4-master2
# -------------------------------------------------------------

module "caasp4-master2" {
    ...
    guest_vm_name           = "caasp4-master2"
    guest_vcpu              = "4"
    guest_memory            = "16384"
    guest_disk0_size        = "40"
    guest_network           = "lab-mgmt"
    guest_ipv4_address      = "192.168.120.112"
...
output "caasp4-master2-VM-ip" {
	value = module.caasp4-master2.VM-ip
}
output "caasp4-master2-VM-guest-ip" {
	value = module.caasp4-master2.VM-guest-ip
}
