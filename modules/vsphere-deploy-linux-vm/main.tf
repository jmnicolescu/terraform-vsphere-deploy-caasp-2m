...
resource "null_resource" "post_install" {
    ...
    provisioner "file" {
        ...
        connection {
            type        = "ssh"
            user        = var.provider_esxi_user
            password    = var.provider_esxi_password
            host        = var.guest_vm_name
        }
  }
  ...
  provisioner "remote-exec" {
      ...
      inline = [
        "chmod +x /root/scripts/01_configure_firewall_access.sh",
        "/root/scripts/01_configure_firewall_access.sh",
        "chmod +x /root/scripts/03_create_users.sh",
        "/root/scripts/03_create_users.sh",
        "chmod +x /root/scripts/05_configure_nfs.sh",
        "/root/scripts/05_configure_nfs.sh",
        "chmod +x /root/scripts/06_configure_rsyslog.sh",
        "/root/scripts/06_configure_rsyslog.sh"
      ]
